...
display_text <- function(Business,Type){
  ...
  if(Type != "menu"){
    ...
    if(length(index_business_match) == 0){
      print(paste('There is neither business id nor business name matching \"', Business, '\".\n', sep = ""))
    }else if(length(which(data_business_match$type == Type)) == 0){
      print(paste('There is no review mentioning your', Type, '\".\n', sep = ""))
    }else{
    ...
    if(length(which(data_business_match$type == "menu")) == 0){
      ...
      if(length(food_list_impressive) != 0){
        output_1 <- 'According to reviews, the following food in your menu are considered to be impressive: \n'
        for(content in food_list_impressive) output_1 <- paste(output_1, content, sep = " ")
      }else output_1 <- 'None of food in your menu is considered to be impressive by reviews. '

      if(length(food_list_to_improve) != 0){
        output_2 <- 'Reviews suggest that the following food in your menu may need to improve: \n'
        for(content in food_list_to_improve) output_2 <- paste(output_2, content, sep = " ")
      }else output_2 <- 'Reviews indicate that none of food in your menu mentioned by reviews have to improve.'
      
      print(paste(output_1, output_2, sep = "; "))
...
display_hist <- function(Business,Type){
  if(Type != "menu"){
    # Read score data
    data <- read.csv("./data/score.csv")
...
server <- function(input, output){
  output$text <- renderUI({
    display_text(Business = input$Business_input, Type = input$Type_input)
  })
  output$Hist <- renderPlot({
    display_hist(Business = input$Business_input, Type = input$Type_input)
  })
}
